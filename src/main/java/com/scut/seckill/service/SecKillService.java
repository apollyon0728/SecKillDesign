...
import com.scut.seckill.utils.SerializeUtil;
...
public class SecKillService {
    ...
    public SecKillEnum handleByPessLockInMySQL(Map<String, Object> paramMap) {
        ...
        long result = jedis.sadd(hasBoughtSetKey, user.getId().toString()); // Set（集合）
        if (result > 0) {
...
//        User user = secKillMapper.getUserById(userId);
    ...
    public User getUserObject(Integer userId) {
        ...
        if (person == null || person.length <= 0) {
            User user = secKillMapper.getUserById(userId);
            setUserObject(userId, user);
            return user;
        } else {
            log.info("{} : 从Redis中命中User对象", userId);
            return (User) SerializeUtil.unserialize(person);
        }
    }
}
